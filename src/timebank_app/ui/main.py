...
def app_main(page: ft.Page) -> None:
    ...
    def sound_options() -> list[ft.dropdown.Option]:
        options = [
            ft.dropdown.Option("", "—"),
            ft.dropdown.Option(RANDOM_SOUND_VALUE, "Случайный звук"),
        ]
    ...
    def parse_bank_seconds(value: str) -> float:
        raw = value.strip()
        if not raw:
            raise ValueError("Укажите число секунд")
        return float(raw)
    ...
    def pause_player_row(player_name: str) -> ft.DataRow:
        ...
        action_control: ft.Control = ft.Text("—")
        if editable:
            ...
            def submit_bank(value: str) -> None:
                try:
                    parsed = parse_bank_seconds(value)
                ...
                apply_pause_edit(
                    player_name,
                    "set_bank",
                    {"player": player_name, "value": parsed},
                )

            bank_field = ft.TextField(
                value=str(int(bank_seconds)),
                width=120,
                on_submit=lambda e: submit_bank(e.control.value),
                on_blur=lambda e: submit_bank(e.control.value),
            )
            bank_control = ft.Row([bank_field, ft.Text(format_mm_ss(bank_seconds))], spacing=8)
            color_button = _button(
                "Цвет",
            ...
            action_control = _button(
                "Удалить",
                lambda _, name=player_name: apply_pause_edit(
                    name,
                    "remove_player",
                    {"player": name},
                ),
            ...
            sound_value = "Случайный звук" if cfg.sound_tap == RANDOM_SOUND_VALUE else cfg.sound_tap
            sound_control = ft.Text(sound_value or "—")
