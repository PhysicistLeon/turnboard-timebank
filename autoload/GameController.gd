...
const EventLoggerScript := preload("res://infra/Logger.gd")
var event_logger = EventLoggerScript.new()
...
func _play_tap_for_current() -> void:
	var current_player_name: String = state.current
	if current_player_name == "" or not state.players.has(current_player_name):
		return
	var p: Model.Player = state.players[current_player_name]
	if p.sound_tap == "":
		return
