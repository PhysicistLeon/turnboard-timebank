...
var sound: SoundService = SoundService.new()

var state: Model.GameState = Model.GameState.new()
var seq: int = 1
...
func _play_tap_for_current() -> void:
	var name: String = state.current
	if name == "" or not state.players.has(name):
		return
	var p: Model.Player = state.players[name]
	if p.sound_tap == "":
		return
	var stream: AudioStream = sound.load_audio_from_path(p.sound_tap)
	if stream != null:
