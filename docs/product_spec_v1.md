...
- Внутреннее время считается с высокой точностью (можно субсекундно), но:
  ...
  - если банк ушёл ниже нуля, формат становится **-MM:SS** (и для 100+ минут остаётся M...:SS без перехода в часы).
...
### 7.1 Обновлённый приоритет формата времени
- Формат `MM:SS` является обязательным и имеет приоритет над предыдущими формулировками отображения в "секундах".
- Все пользовательские экраны (Game, Setup, Tech Pause) используют единый форматтер времени.
