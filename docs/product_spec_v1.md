...
- Обновлённый приоритетный формат UI-таймера: **строго `MM:SS`**; при исчерпании банка отображение `-MM:SS` (для минут >99 используется естественное расширение `MMM:SS`, `MMMM:SS` и т.д.).
- Внутреннее время считается с высокой точностью (можно субсекундно), но:
  - на UI таймер отображается **в секундах** (округление/усечение — на выбор, но должно быть стабильным и не “скакать”).
